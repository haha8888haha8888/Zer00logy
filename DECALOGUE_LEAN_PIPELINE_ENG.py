...
class LeanRunner:
    ...
    def run(self, lean_code: str) -> LeanRunResult:
        ...
        if lean_path is None:
            return LeanRunResult(LeanExecutionStatus.NOT_INSTALLED,
                "Lean 4 executable was not found in PATH. Candidate generated; formal verification not executed.")
        with tempfile.TemporaryDirectory(prefix="decalogue_lean_") as tmpdir:
            lean_file = Path(tmpdir) / "GeneratedProof.lean"
            lean_file.write_text(lean_code, encoding="utf-8")
            try:
                completed = subprocess.run([lean_path, str(lean_file)], capture_output=True,
                    text=True, timeout=self.timeout_seconds, check=False)
            except subprocess.TimeoutExpired as exc:
                return LeanRunResult(LeanExecutionStatus.TIMEOUT,
                    f"Lean execution exceeded {self.timeout_seconds} seconds.\n{exc}")
            except OSError as exc:
                return LeanRunResult(LeanExecutionStatus.EXECUTION_ERROR,
                    f"Lean process could not be started: {exc}")
            combined = "\n".join(s.strip() for s in (completed.stdout, completed.stderr) if s.strip())
            if completed.returncode == 0:
                return LeanRunResult(LeanExecutionStatus.VERIFIED,
                    combined or "Lean completed successfully with no output.", completed.returncode)
            return LeanRunResult(LeanExecutionStatus.REJECTED,
                combined or "Lean rejected the candidate without diagnostic output.", completed.returncode)
